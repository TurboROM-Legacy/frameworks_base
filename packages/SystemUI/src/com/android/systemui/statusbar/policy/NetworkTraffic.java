...
public class NetworkTraffic extends TextView {
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.System
                    ...
                    this, UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System
                    .getUriFor(Settings.System.NETWORK_TRAFFIC_COLOR_OVERRIDE), false,
                    this, UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System
    ...
    final void updateNetworkTrafficColor() {
    ...
    private void updateSettings() {
        ...
        if (overrideTrafficColor) {
            mNetworkTrafficColor = trafficColor;
        } else {
    ...
    private void updateTrafficDrawable(int trafficColor) {
        int trafficDrawable;
        Drawable trafficIcon = null;
        if (!mHideArrow) {
            if (isSet(mState, MASK_UP + MASK_DOWN)) {
                trafficDrawable = R.drawable.stat_sys_network_traffic_updown;
            } else if (isSet(mState, MASK_UP)) {
                trafficDrawable = R.drawable.stat_sys_network_traffic_up;
            } else if (isSet(mState, MASK_DOWN)) {
                trafficDrawable = R.drawable.stat_sys_network_traffic_down;
            } else {
                trafficDrawable = 0;
            }
            if (trafficDrawable != 0) {
                trafficIcon = getResources().getDrawable(trafficDrawable);
                trafficIcon.setColorFilter(null);
                trafficIcon.setColorFilter(trafficColor, PorterDuff.Mode.SRC_ATOP);
            }
        } else {
            trafficIcon = null;
        }
    ...
    public void setColor(int trafficColor) {
        ContentResolver resolver = mContext.getContentResolver();
        int defaultColor = mContext.getResources().getColor(R.color.status_bar_traffic_color);
        mNetworkTrafficColor = Settings.System.getIntForUser(resolver,
                Settings.System.NETWORK_TRAFFIC_COLOR, defaultColor,
                UserHandle.USER_CURRENT);
        updateSettings();
    }

    public void setColor2(float darkIntensity) {
        mIconTint = getColorForDarkIntensity(darkIntensity, mLightModeFillColor, mDarkModeFillColor);
        updateSettings();
    }
