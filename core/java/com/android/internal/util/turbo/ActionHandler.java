...
public class ActionHandler {
    ...
    public static class ActionIconResources {
        ...
        public ActionIconResources(Resources res) {
            ...
            for (int i = 0; i < systemActions.length; i++) {
                ...
                mDrawables[i] = TurboActionUtils.getDrawable(res, systemActions[i].mIconRes,
        ...
        for (int i = 0; i < systemActions.length; i++) {
            ...
            if (TextUtils.equals(action, SYSTEMUI_TASK_STOP_SCREENPINNING)
                ...
                continue;
                    ...
                    && !TurboActionUtils.deviceSupportsMobileData(context)) {
                continue;
            } else if (TextUtils.equals(action, SYSTEMUI_TASK_BT)
                    && !TurboActionUtils.deviceSupportsBluetooth()) {
                continue;
            } else if (TextUtils.equals(action, SYSTEMUI_TASK_TORCH)
                 && !TurboActionUtils.deviceSupportsFlashLight(context)) {
                continue;
                ...
                if (!TurboActionUtils.getBoolean(context, "config_enableScreenrecordChord",
                        TurboActionUtils.PACKAGE_ANDROID)) {
                    continue;
                }
            }
            bundle.add(c);
    ...
    public static void performTaskFromKeyguard(Context ctx, String action) {
        ...
        if (!action.startsWith(SYSTEM_PREFIX)) {
            Intent intent = TurboActionUtils.getIntent(action);
            if (intent == null) {
                return;
            }
    ...
    private static void switchToLastApp(Context context) {
        ...
        if (lastTask != null) {
            final ActivityOptions opts = ActivityOptions.makeCustomAnimation(context,
                    TurboActionUtils.getIdentifier(context, "last_app_in", "anim", TurboActionUtils.PACKAGE_ANDROID),
                    TurboActionUtils.getIdentifier(context, "last_app_out", "anim", TurboActionUtils.PACKAGE_ANDROID));
