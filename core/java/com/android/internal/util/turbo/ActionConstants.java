...
import com.android.internal.util.turbo.ActionHandler.SystemAction;
import com.android.internal.util.turbo.Config.ActionConfig;
    ...
    public static class Fling implements Defaults {
        ...
        static {
            defMap.put(CONFIG_fling_touchslop_increase_factor, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_fling_touchslop_increase_factor,
                    TurboActionUtils.FORMAT_FLOAT, TurboActionUtils.DIMEN));
            defMap.put(CONFIG_FlingLongSwipePortraitLeft, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_FlingLongSwipePortraitLeft,
                    TurboActionUtils.FORMAT_FLOAT, TurboActionUtils.DIMEN));
            defMap.put(CONFIG_FlingLongSwipePortraitRight, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_FlingLongSwipePortraitRight,
                    TurboActionUtils.FORMAT_FLOAT, TurboActionUtils.DIMEN));
            defMap.put(CONFIG_FlingLongSwipeLandscapeLeft, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_FlingLongSwipeLandscapeLeft,
                    TurboActionUtils.FORMAT_FLOAT, TurboActionUtils.DIMEN));
            defMap.put(CONFIG_FlingLongSwipeLandscapeRight, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_FlingLongSwipeLandscapeRight,
                    TurboActionUtils.FORMAT_FLOAT, TurboActionUtils.DIMEN));
            defMap.put(CONFIG_FlingLongSwipeVerticalUp, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_FlingLongSwipeVerticalUp,
                    TurboActionUtils.FORMAT_FLOAT, TurboActionUtils.DIMEN));
            defMap.put(CONFIG_FlingLongSwipeVerticalDown, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_FlingLongSwipeVerticalDown,
                    TurboActionUtils.FORMAT_FLOAT, TurboActionUtils.DIMEN));
            defMap.put(CONFIG_pulsePathEffect_1, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_pulsePathEffect_1, TurboActionUtils.DIMEN_PIXEL));
            defMap.put(CONFIG_pulsePathEffect_2, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_pulsePathEffect_2, TurboActionUtils.DIMEN_PIXEL));
            defMap.put(CONFIG_pulsePathStrokeWidth, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_pulsePathStrokeWidth, TurboActionUtils.DIMEN_PIXEL));
            defMap.put(CONFIG_pulseFillColor, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_pulseFillColor, TurboActionUtils.COLOR));
            defMap.put(CONFIG_pulseDivisions, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_pulseDivisions, TurboActionUtils.INT));
            defMap.put(CONFIG_pulseDbFuzzFactor, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_pulseDbFuzzFactor, TurboActionUtils.INT));
            defMap.put(CONFIG_pulseDbFuzz, new ConfigHolder(
                    TurboActionUtils.PACKAGE_SYSTEMUI, CONFIG_pulseDbFuzz, TurboActionUtils.INT));
        }
...
*/
    ...
    private static Bundle loadConfigsFromMap(Context ctx, Map<String, ConfigHolder> configMap) {
        ...
        for (Map.Entry<String, ConfigHolder> entry : configMap.entrySet()) {
            ...
            Object obj = TurboActionUtils.getValue(ctx, holder.name, holder.type, holder.format,
                    holder.pkg);
            TurboActionUtils.putValue(holder.name, obj, holder.type, b);
        }
        return b;
    }
