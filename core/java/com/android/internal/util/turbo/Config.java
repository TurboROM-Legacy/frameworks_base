...
import com.android.internal.util.turbo.ActionHandler;
import com.android.internal.util.turbo.ActionConstants.ConfigMap;
import com.android.internal.util.turbo.ActionConstants.Defaults;
...
public class Config {
    ...
    public static class ActionConfig implements Stringable, Comparable<ActionConfig>, Parcelable {
        ...
        public ActionConfig(Context ctx) {
            label = TurboActionUtils.getFriendlyNameForUri(ctx, action);
        }
        ...
        public Drawable getDefaultIcon(Context ctx) {
            return TurboActionUtils.getDrawableForAction(ctx, action);
        }
        ...
        public Drawable getCurrentIcon(Context ctx) {
            if (!hasCustomIcon()) {
                ...
                if (TextUtils.equals(type, "iconpack") && items.size() == 3) {
                    ...
                    Drawable d = TurboActionUtils.getDrawable(ctx, iconName, packageName);
                    if (d == null) {
                        d = TurboActionUtils.getDrawableForAction(ctx, action);
                    }
                    return d;
                } else {
                    return TurboActionUtils.getDrawableForAction(ctx, action);
